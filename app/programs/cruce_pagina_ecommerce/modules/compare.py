...
def _filter(header, data):
    ...
    for line in data:
        if float(line[pim_index]) and int(line[repeated_index]) == 1 and int(line[activate_index]) == 1 and line[active2_index] and line[show_index]:
            values = [line[style_color_index], int(line[available_qty_index]), int(line[sap_stock_index])]
