                ...
                if row == 1:
                    ...
                    for f in [fecha_con_index, fecha_con_index, antiguedad_index]:
                        ...
                        if serial and '/' not in serial:
                            # print(serial)
                            fecha = datetime.utcfromtimestamp((float(serial) - 25569) * 86400.0)
...
def read_param(fs, path):  # Leer parametros
    ...
    P = namedtuple("Parameters", ['llave_emp', 'empresas', 'temporada', 'genero', 'descripcion', 'pais', 'marca2', 'descripcionC', 'tiendas', 'tipo_cambio', 'tipo_doc'])
    empresa_dict = {}
    llave_emp_dict = {}
    temporada_dict = {}
    genero_dict = {}
            ...
            # for row in range(1, 15):
                ...
                if row == 0:
                    ...
                    if col_value[empresa_index]:
                        empresa_dict[col_value[empresa_index]] = [col_value[empresa_index + 1], col_value[empresa_index + 2], col_value[empresa_index + 3]]
                    if col_value[llave_emp_index]:
                        llave_emp_dict[col_value[llave_emp_index]] = col_value[llave_emp_index:llave_emp_index + 6]
                    if col_value[temporada_index]:
                        temporada_dict[col_value[temporada_index]] = col_value[temporada_index + 1]
                    if col_value[genero_index]:
                        genero_dict[col_value[genero_index]] = col_value[genero_index + 1]
                    if col_value[pais_corr_index]:
                        pais_dict[col_value[pais_corr_index - 1]] = col_value[pais_corr_index]
                    if col_value[marca2_index]:
                        marca2_dict[col_value[marca2_index]] = col_value[marca2_index + 1]
                    if col_value[descripcionC_index]:
                        descripcionC_dict[col_value[descripcionC_index].upper()] = col_value[descripcionC_index + 1: descripcionC_index + 4]
                    if col_value[tienda_index]:
                        tiendas_dict[col_value[tienda_index]] = col_value[tienda_index + 1: tienda_index + 3]
                    if col_value[tipo_cambio_index]:
                        ...
                        tipo_cambio_dict[key] = col_value[tipo_cambio_index + 1: tipo_cambio_index + 3]
                    if col_value[tipo_doc_index]:
                        tipo_doc_dict[col_value[tipo_doc_index].upper()] = col_value[tipo_doc_index + 1]
    param = P(llave_emp_dict, empresa_dict, temporada_dict, genero_dict, descripcion_dict, pais_dict, marca2_dict, descripcionC_dict, tiendas_dict, tipo_cambio_dict, tipo_doc_dict)
    # print(param.tiendas)
    # exit()
    return param
...
def create_macro(header1, data, param):  # realizar el trabajo de la macro
    ...
    for line in data:
        ...
        pairs = [('CantFacturada', 'Cantidad'), ('Anio', 'Año Temporada'), ("Semana", 'Número Semana'), ('Marca2', 'Marca Corr'), ('Total Final', 'Venta Neta'), ('Costo Rep', 'Costo Unitario'), ('HoraVentaRetail', 'Hora Venta')]
        ...
        ultimo_dia_mes = f'{ultimo_dia_mes.month}/{ultimo_dia_mes.day}/{ultimo_dia_mes.year}'
        ...
        for col in ['Venta Neta', 'Costo Unitario', 'Costo de Venta', 'Contrib', 'Venta Pesos', 'Costo Pesos', 'Contrib Pesos']:
...
def write(fs, header, data, name="out", limit=None):  # Crear csv para poder copiar en matriz
