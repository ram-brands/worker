...
def build_key(ref_origin_dest_zone, basecodes, bill, ref_vol_weight):
    ...
    for ref in bill:
        try:
            ...
            dest_code = str(float(ref_origin_dest_zone[ref].destination_code)).replace(".0", "")
...
def build_key2(bill, ref_order, order_sku, store_city, basecodes, ref_vol_weight, initials_region_code, initials_to_zone):
    print('Creando Keys...')
    Key = namedtuple("Key", ['origin', 'zone', 'dest_code', 'weight_code'])
    ref_key = {}
    error = set()
