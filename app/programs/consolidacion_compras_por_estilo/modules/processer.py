...
def process(final_header, header, data, stores): # Format to final header and then split by store
    ...
    for file_ in data:
        ...
        for line in file_.data:
            ...
            new_line[final_header.index("DELIVERY (MES)")] = line[header.index('DELIVERY (MES)')]
            ...
            new_line[final_header.index("CLASE (COMPRADOR)")] = line[header.index('CLASE')]
            ...
            try:
                ...
                new_line[final_header.index("CLASE (PROVEEDOR)")] = line[header.index('PRODUCT SUBCLASS')]
...
def add_parameters(final_header, data_header, data, parameters): # Add value de parameter fields
    final_data = []
    fl_errors = set()
    errors = set()
    errors2 = set()
